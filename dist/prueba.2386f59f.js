var currentImg = undefined, currentImgProps = {
    x: 0,
    y: 0
}, isZooming = false, column = -1, mouse = {
    x: 0,
    y: 0
}, delayedPlay;
for(var i = 0; i < 12; i++){
...
function pauseBoxes(b) {
    ...
    for(var i1 = 0; i1 < $('.mainBoxes').children().length; i1++){
        var b = $('.mainBoxes').children()[i1];
...
function playBoxes() {
    for(var i2 = 0; i2 < $('.mainBoxes').children().length; i2++){
        var tl = $('.mainBoxes').children()[i2].tl;
        tl.play();
        gsap.to(tl, {
            duration: 0.4,
            timeScale: 1,
...
window.onload = function() {
    ...
    if (!!('ontouchstart' in window)) {
        ...
        mouse.y = 60;
    } else $('.main').on('mousemove', function(e) {
        mouse.x = e.x;
        mouse.y = e.layerY;
...
//# sourceMappingURL=prueba.2386f59f.js.map
